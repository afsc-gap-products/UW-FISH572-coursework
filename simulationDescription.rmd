...
dat <- readRDS("coursework/simulations/sim_data/sim_dat_10.RDS")
...
mesh <- sdmTMB::make_mesh(samples, 
                          xy_cols = c("X", "Y"), 
                          type = "cutoff_search", 
                          n_knots = 50) # try a bit higher too
plot(mesh)
...
fit <- sdmTMB::sdmTMB(
  ...
  # turn spatiotemporal = "off" if get errors about sigma_E 
)
...
# spatial random effects (if estimated, if not, comment out)
plot_map(predictions$data, omega_s) + 
  ggtitle("Spatial random effects only") + 
  scale_fill_gradient2()

# spatiotemporal random effects (if estimated, if not, comment out)
...
index <- sdmTMB::get_index(predictions, area = 1, bias_correct = TRUE, level = 0.95) # desired confidence interval here is 95% 
