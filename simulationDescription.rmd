...
Assuming again that the area of each grid cell is 1, under simple random
sampling the design-based estimate is the product of the mean of observations 
and the total area (1 * the number of grid cells in the domain).
```{r index}
index_db <- theSample |> 
  group_by(year) |> 
  summarize(index = mean(observation) * 10000, 
            variance = )
```